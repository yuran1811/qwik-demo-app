...
export default component$<{ fieldErrors?: Record<string, string>; success?: boolean; errorMessage?: string }>(
  ({ fieldErrors, success, errorMessage }) => {
    return (
        ...
        <div class="space-y-12">
          <div class="space-y-4">
            <div>
              ...
              />
              {fieldErrors && fieldErrors?.['username'] && <p class="form-err">{fieldErrors['username']}</p>}
            </div>
            <div>
              ...
              <input
                ...
                class="w-full rounded-md border border-gray-700 bg-gray-900 px-3 py-2 text-gray-100 dark:border-gray-300 dark:bg-gray-50 dark:text-gray-800"
              />
              {fieldErrors && fieldErrors?.['password'] && <p class="form-err">{fieldErrors['password']}</p>}
            </div>

            {!!success && <p class="text-green-500 dark:text-green-600">Account created successfully</p>}
            {!success && !!errorMessage && <p class="form-err">{errorMessage}</p>}
          </div>
          <div class="space-y-2">
            <div>
              <button
